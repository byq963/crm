...
<%@page import="com.prj.biz.impl.SysRoleRightBiz"%>
<%@page import="com.prj.biz.ISysRoleRightBiz"%>
<%@page import="com.prj.biz.ISysRoleBiz"%>
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%@page import="com.prj.bean.SysRole"%>
<%@page import="com.prj.bean.SysRight"%>
<%@page import="com.prj.bean.SysUser"%>
<%@page import="com.prj.bean.SysRoleRight"%>
<%
...
<script language="JavaScript">function getDHTMLObj(docName, objName) {return eval(docName + theBrowser.DHTMLRange + '.' + objName + theBrowser.DHTMLStyleObj);}
...
function initialise() {
	...
	ISysRoleRightBiz roleRightBiz=(ISysRoleRightBiz)applicationContext.getBean("sysRoleRightBizBean");
		List<SysRoleRight> sysRoleRights=roleRightBiz.showSysRoleRightById(role.getRoleId());
		for(SysRoleRight right : sysRoleRights){
			//获取权限的编号
			String code = right.getSysRight().getRightCode();
			//获取父节点s的编号
			String parentCode = right.getSysRight().getRightParentCode();
			if(code.length()==3){%>
		
				a= theMenu.addChild(l2ID,'<%="ROOT_MENU".equals(right.getSysRight().getRightParentCode())?"Folder":"Document"%>', "<%=right.getSysRight().getRightText()%>","<%="ROOT_MENU".equals(right.getSysRight().getRightParentCode())?"":right.getSysRight().getRightUrl()%>", "<%=right.getSysRight().getRightTip()%>");
		
			<%}
			if(parentCode.length()==3){%>
				theMenu.addChild(a,'<%="ROOT_MENU".equals(right.getSysRight().getRightParentCode())?"Folder":"Document"%>', "<%=right.getSysRight().getRightText()%>", "<%=right.getSysRight().getRightUrl()%>", "<%=right.getSysRight().getRightTip()%>");
			<%}%>
	<%}%>  
...
<SCRIPT LANGUAGE="JavaScript">if (self.name == 'menu') {
...
<NOSCRIPT>
	<BODY BGCOLOR="#FFFFCC">
		...
		<P>If you have any problems with this site, please contact the
			Webmaster.</P>
	</BODY>
</NOSCRIPT>
