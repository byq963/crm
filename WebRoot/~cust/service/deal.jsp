...
<script type="text/javascript">
	function goreload(){
		...
		if(page<(allpage+1)){
		window.location.href="serviceManagerAction!showAllNeadDeal?page="+page+"&pagesize="+pagesize;
		}else{
			alert("输入的页数超过最大页数");
		}
	}
	function keydown(){
		...
		window.location.href="serviceManagerAction!showAllNeadDeal?page="+1+"&pagesize="+pagesize;
	}
	function query(){
		form1.submit();
	}
...
<form action="serviceManagerAction!showAllNeadDeal" method="post" name="form1">
<table class="query_form_table">
	<tr>
		<th height="28">客户</th>
		<td><input name="servCustName" value="${servCustName}"/></td>
		<th height="28">概要</th>
		<td><input  name="servTitle" value="${servTitle}"/></td>
	...
	<tr>
		...
		<td colspan="3">
			<input name="servCreateDate1" value="${servCreateDate1}" size="10" /> - <input name="servCreateDate2" value="${servCreateDate2}" size="10" /></td>
		<th height="32">状态</th>
		<td>
...
<table class="data_list_table">
	...
	<tr>
		...
		<td class="list_data_op">
			...
			<img onclick="window.location.href='serviceManagerAction!dealInput?servNo=${service1.servNo}'" title="处理" src="images/bt_deal.gif" class="op_button" />  
...
<div class="pager">
	...
	<a href="serviceManagerAction!showAllNeadDeal?page=1&pagesize=${pagesize}">第一页</a>
	<c:if test="${page>1}">
	<a href="serviceManagerAction!showAllNeadDeal?page=${page-1}&pagesize=${pagesize}">上一页</a>
	</c:if>
	<c:if test="${page<2}">
	<a href="serviceManagerAction!showAllNeadDeal?page=${page}&pagesize=${pagesize}">上一页</a>
	</c:if>
	<c:if test="${requestScope.allpage>page}">
	<a href="serviceManagerAction!showAllNeadDeal?page=${page+1}&pagesize=${pagesize}">下一页</a>
	</c:if>
	<c:if test="${requestScope.allpage<page+1}">
	<a href="serviceManagerAction!showAllNeadDeal?page=${requestScope.allpage}&pagesize=${pagesize}">下一页</a>
	</c:if>
	<a href="serviceManagerAction!showAllNeadDeal?page=${requestScope.allpage}&pagesize=${pagesize}">最后一页</a>
