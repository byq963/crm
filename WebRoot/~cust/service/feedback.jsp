...
<script type="text/javascript">
	function goreload(){
		...
		if(page<(allpage+1)){
		window.location.href="serviceManagerAction!feedback?page="+page+"&pagesize="+pagesize;
		}else{
			alert("输入的页数超过最大页数");
		}
	}
	function keydown(){
		...
		window.location.href="serviceManagerAction!feedback?page="+1+"&pagesize="+pagesize;
	}
	function query(){
		form1.submit();
	}
...
<form action="serviceManagerAction!feedback" method="post" name="form1">
...
<table class="data_list_table">
	...
	<tr>
		...
		<td class="list_data_op">
			...
			<img onclick="window.location.href='serviceManagerAction!reflectInput?servNo=${service1.servNo}'" title="处理" src="images/bt_feedback.gif" class="op_button" />  
...
<div class="pager">
	...
	<a href="serviceManagerAction!feedback?page=1&pagesize=${pagesize}">第一页</a>
	<c:if test="${page>1}">
	<a href="serviceManagerAction!feedback?page=${page-1}&pagesize=${pagesize}">上一页</a>
	</c:if>
	<c:if test="${page<2}">
	<a href="serviceManagerAction!feedback?page=${page}&pagesize=${pagesize}">上一页</a>
	</c:if>
	<c:if test="${requestScope.allpage>page}">
	<a href="serviceManagerAction!feedback?page=${page+1}&pagesize=${pagesize}">下一页</a>
	</c:if>
	<c:if test="${requestScope.allpage<page+1}">
	<a href="serviceManagerAction!feedback?page=${requestScope.allpage}&pagesize=${pagesize}">下一页</a>
	</c:if>
	<a href="serviceManagerAction!feedback?page=${requestScope.allpage}&pagesize=${pagesize}">最后一页</a>
